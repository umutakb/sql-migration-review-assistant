...
def test_not_null_without_default_rule_ignores_identity_column(default_config) -> None:
    migration = MigrationFile(
        path="x.sql",
        relative_path="x.sql",
        content=(
...
def test_not_null_without_default_rule_flags_default_null(default_config) -> None:
    migration = MigrationFile(
        path="x.sql",
        relative_path="x.sql",
        content="ALTER TABLE users ADD COLUMN created_by TEXT NOT NULL DEFAULT NULL;",
    )
    parse_migration_file(migration, "postgres")
